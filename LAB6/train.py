...
def build_firstPart_model(input_var,keep_prob_=0.5):
		...
		# Convolution
		network = tf.layers.conv1d(inputs=input_var, filters=64, kernel_size=50, strides=6, padding='same', activation=tf.nn.relu)
		...
		# Convolution
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=8, strides=1, padding='same', activation=tf.nn.relu)
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=8, strides=1, padding='same', activation=tf.nn.relu)
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=8, strides=1, padding='same', activation=tf.nn.relu)
		...
		# Convolution
		network = tf.layers.conv1d(inputs=input_var, filters=64, kernel_size=400, strides=50, padding='same', activation=tf.nn.relu)
		network = tf.layers.max_pooling1d(inputs=network, pool_size=4, strides=4, padding='same')
		...
		# Convolution
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=6, strides=1, padding='same', activation=tf.nn.relu)
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=6, strides=1, padding='same', activation=tf.nn.relu)
		network = tf.layers.conv1d(inputs=network, filters=128, kernel_size=6, strides=1, padding='same', activation=tf.nn.relu)
...
def build_network(hparams,char2numY,inputs,dec_inputs,keep_prob_=0.5,):
	...
	with tf.variable_scope("encoding") as encoding_scope:
		if not hparams.bidirectional:
			# If you want to change the bidirectional to unidirectional
	...
	with tf.variable_scope("decoding") as decoding_scope:
		...
		if hparams.use_attention:
			...
			attention_mechanism = tf.contrib.seq2seq.LuongAttention(hparams.num_units * 2 if hparams.bidirectional else hparams.num_units , encoder_outputs, memory_sequence_length=None)
			decoder_cells = tf.contrib.seq2seq.AttentionWrapper(decoder_cells, attention_mechanism, attention_layer_size=hparams.attention_size,alignment_history=True)
		...
		decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cells, training_helper, encoder_state, output_layer=output_layer)
	...
	if not hparams.use_beamsearch_decode:
		inference_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(decoder_embedding, start_tokens,end_token)

		# Inference Decoder
		inference_decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cells, inference_helper, encoder_state, output_layer=output_layer)
	...
	# Dynamic decoding
	outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(inference_decoder,impute_finished = False, maximum_iterations=hparams.output_max_length)
...
def run_program(hparams,FLAGS):
	...
	def evaluate_model(hparams, X_test, y_test, classes):
		...
		for index_ in range(n_classes):
			print( "\t{} rhythm -> Sensitivity: {:1.4f}, Specificity: {:1.4f}, Precision (PPV): {:1.4f}, F1 : {:1.4f} Accuracy: {:1.4f}".format(
				classes[index_], sensitivity[index_], specificity[index_], PPV[index_], f1[index_], acc[index_]))

		print("\tAverage -> Sensitivity: {:1.4f}, Specificity: {:1.4f}, Precision (PPV): {:1.4f}, F1-score: {:1.4f}, Accuracy: {:1.4f}".format(
		...
		with tf.Graph().as_default(), tf.Session() as sess:
			...
			if ckpt_exist:
				...
				for epoch_i in range(hparams.epochs):
					...
					print('Epoch {:3} Loss: {:>6.3f} Accuracy: {:>6.4f} F1-score: {:>6.4f} Cohen\'s Kappa: {:>6.4f} Epoch duration: {:>6.3f}s'.format(
																		epoch_i, np.mean(batch_loss), accuracy,mf1,ck_score, time.time() - start_time))
					...
					if (epoch_i+1)%hparams.test_step==0:
						...
						if np.nan_to_num(f1_macro) > pre_f1_macro: # save the better model based on the f1 score
							print('Loss {:.4f} after {} epochs (batch_size={})'.format(loss_track[-1], epoch_i + 1, hparams.batch_size))
...
def main():
	...
	parser.add_argument("--data_dir", type=str, default="data_2013/eeg_fpz_cz", help="Directory where to load training data.")
	parser.add_argument("--output_dir", type=str, default="data_2013/eeg_fpz_cz", help="Directory where to save trained models and outputs.")
	parser.add_argument("--num_folds", type=int, default=20, help="Number of cross-validation folds.")
	parser.add_argument("--checkpoint_dir", type=str, default="checkpoints-seq2seq-sleep-EDF", help="Directory to save checkpoints")
	...
	hparams = tf.contrib.training.HParams(
		...
		num_units=128, #fixed
		embed_size=10, #fixed
		input_depth=3000, #fixed
		n_channels=100, #fixed
		bidirectional=True, 
		use_attention=True,
		lstm_layers=2, #fixed
		attention_size=64, #fixed
		beam_width=4,
		use_beamsearch_decode=False,
		max_time_step=10, #fixed
		output_max_length=10 + 2, #fixed
		test_step=5, #fixed
	)

	if args.data_dir is not None:
		run_program(hparams,args)
